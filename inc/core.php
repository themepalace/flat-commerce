...
if ( ! function_exists( 'flat_commerce_scripts' ) ) :
	...
	function flat_commerce_scripts() {
		...
		$flat_commerce_deps = false;

		wp_enqueue_style( 'flat-commerce-style', get_stylesheet_uri(), $flat_commerce_deps, FLAT_COMMERCE_THEME_VERSION );

		wp_enqueue_style( 'flat-commerce-responsive', get_template_directory_uri() . '/css/responsive.css', $flat_commerce_deps, FLAT_COMMERCE_THEME_VERSION );
...
if ( ! function_exists( 'flat_commerce_custom_css' ) ) :
	...
	function flat_commerce_custom_css() {
		...
		if ( ( !$flat_commerce_custom_css = get_transient( 'flat_commerce_custom_css' ) ) ) {
			...
			if ( 'blank' == $text_color ){
				$flat_commerce_custom_css	.=  ".site-title a, .site-description { position: absolute !important; clip: rect(1px 1px 1px 1px); clip: rect(1px, 1px, 1px, 1px); }". "\n";
			}
			elseif ( HEADER_TEXTCOLOR != $text_color ) {
				$flat_commerce_custom_css	.=  ".site-title a, .site-title a:hover, .site-description { color: #".  $text_color ."; }". "\n";
			}
	...
	function flat_commerce_post_navigation() {
		...
		if ( !$disable_single_post_navigation ) {
			...
			the_post_navigation( array(
				'next_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Next &rarr;', 'flat-commerce' ) . '</span> ' .
					'<span class="screen-reader-text">' . __( 'Next post:', 'flat-commerce' ) . '</span> ' .
					'<span class="post-title">%title</span>',
				'prev_text' => '<span class="meta-nav" aria-hidden="true">' . __( '&larr; Previous', 'flat-commerce' ) . '</span> ' .
					'<span class="screen-reader-text">' . __( 'Previous post:', 'flat-commerce' ) . '</span> ' .
					'<span class="post-title">%title</span>',
			) );
		}
	}
...
if ( ! function_exists( 'flat_commerce_continue_reading' ) ) :
	...
	function flat_commerce_continue_reading() {
		...
		return ' <a class="more-link" href="' . esc_url( get_permalink() ) . '">' .  sprintf( __( '%s', 'flat-commerce' ) , $more_tag_text ) . '</a>';
	}
...
function flat_commerce_footer_content() {
	...
	$search = array( '[the-year]', '[site-link]' );
    	...
    	<div id="site-generator">
    		<div class="wrapper">
    			<div id="footer-content" class="copyright">'.__( 'Copyright &copy; ', 'flat-commerce' ).date( 'Y' ).'</a>. '.__( 'All Rights Reserved', 'flat-commerce' ).'</div>
    		</div><!-- .wrapper -->
		</div><!-- #site-generator -->';
	...
	function flat_commerce_post_navigation() {
		...
		the_post_navigation( array(
			'next_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Next &rarr;', 'flat-commerce' ) . '</span> ' .
				'<span class="screen-reader-text">' . __( 'Next post:', 'flat-commerce' ) . '</span> ' .
				'<span class="post-title">%title</span>',
			'prev_text' => '<span class="post-title">%title</span><span class="meta-nav" aria-hidden="true">' . __( ' &larr; Previous', 'flat-commerce' ) . '</span> ' .'<span class="screen-reader-text">' . __( 'Previous post:', 'flat-commerce' ) . '</span> ',
		) );
	}
endif; //flat_commerce_post_navigation
add_action( 'flat_commerce_after_post', 'flat_commerce_post_navigation', 10 );
