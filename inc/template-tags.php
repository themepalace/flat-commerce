...
function flat_commerce_posted_on() {
	...
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '<time class="entry-date published" datetime="%1$s"><span>%2$s</span><span>%3$s</span></time><time class="entry-date updated" datetime="%4$s">%5$s</time>';
	}
	...
	$byline = sprintf(
		esc_html_x( 'By %s', 'post author', 'flat-commerce' ),
		esc_html( get_the_author() )
	);

	echo '<span class="genericon genericon-user author vcard" title="' . esc_attr( $byline ) .'"></span>';
}
endif;
